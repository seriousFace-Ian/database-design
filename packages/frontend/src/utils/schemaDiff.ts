...
function diffTable(
  ...
  for (const [name, tField] of tgtFieldMap) {
    ...
    if (cn.defaultValue !== tn.defaultValue)
      changes.default = {from: cn.defaultValue, to: tn.defaultValue}
  ...
  // PK：按字段名集合比较
  const curPk = cur.fields
    .filter(f => f.isPrimaryKey)
    .map(f => f.name)
    .sort()
  const tgtPk = tgt.fields
    .filter(f => f.isPrimaryKey)
    .map(f => f.name)
    .sort()
  const pkChanged =
  ...
  const commentChanged = curComment !== tgtComment ? {from: curComment, to: tgtComment} : null
  ...
  for (const m of diff.tables.modified) {
    ...
    for (const fk of m.fksDropped) {
      sections.alters.push(
        `ALTER TABLE ${qname} DROP CONSTRAINT IF EXISTS ${quoteIdent(fk.constraintName)};`
      )
    }
...
function renderColumnModification(qname: string, cm: FieldChange): string[] {
  ...
  if (cm.changes.unique) {
    if (cm.changes.unique.to) {
      ...
      out.push(`ALTER TABLE ${qname} DROP CONSTRAINT IF EXISTS ${quoteIdent(`uq_${cm.field}`)};`)
    }
  }
  return out
}
...
export function countDiffChanges(diff: SchemaDiff): {
  ...
  for (const m of diff.tables.modified) {
    added +=
      m.columnsAdded.length +
      m.fksAdded.length +
      m.indexesAdded.length +
      m.tableConstraintsAdded.length
    dropped +=
      m.columnsDropped.length +
      m.fksDropped.length +
      m.indexesDropped.length +
      m.tableConstraintsDropped.length
