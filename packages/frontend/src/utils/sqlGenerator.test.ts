...
import {generateDdlSections, generateDdlStatements, generateProjectDdl} from './sqlGenerator'
...
describe('ENUM', () => {
  ...
  it('引用缺失的 ENUM 时退化为 TEXT', () => {
    const p = project({
      tables: [table({fields: [field({name: 'x', type: 'USER-DEFINED', enumTypeId: 'missing'})]})],
    })
...
describe('外键 / 拓扑排序', () => {
  ...
  it('被引用表先于引用表创建', () => {
    ...
    const names = generateDdlSections(p).tables.map(
      t => t.match(/CREATE TABLE "public"."(\w+)"/)![1]
    )
...
describe('IDENTITY 列', () => {
  ...
  it('ALWAYS AS IDENTITY 输出', () => {
    ...
    expect(generateDdlSections(p).tables[0]).toContain('"id" INTEGER GENERATED ALWAYS AS IDENTITY')
  })
...
describe('整体 DDL', () => {
  ...
  it('generateProjectDdl 含分节注释且以换行结尾', () => {
    const p = project({
      tables: [
        table({fields: [field({name: 'id', type: 'SERIAL', isPrimaryKey: true, nullable: false})]}),
      ],
    })
    ...
    const p = project({
      tables: [
        table({
          name: 't',
          fields: [fid, fa, fb],
          constraints: [{id: 'c1', kind: 'UNIQUE', fieldIds: ['fa', 'fb']}],
        }),
      ],
    })
    const ddl = generateDdlSections(p).tables[0]
