...
import type {EnumType, FieldDefinition, ProjectFile, TableDefinition} from '@/types/schema'
...
describe('schemaDiff', () => {
  ...
  it('detects added and dropped columns', () => {
    ...
    expect(sql).toContain(
      'ALTER TABLE "public"."users" ADD COLUMN "new_col" VARCHAR(100) NOT NULL;'
    )
  })
  ...
  it('detects added foreign key with constraint name', () => {
    const users = table({
      ...
      name: 'users',
      fields: [
        field({id: 'uid', name: 'id', type: 'BIGSERIAL', isPrimaryKey: true, nullable: false}),
      ],
    })
  ...
  it('detects new ENUM and added ENUM value', () => {
    const enumBefore: EnumType = {
      id: 'e1',
      schema: 'public',
      name: 'order_status',
      values: ['pending', 'paid'],
    }
    const enumAfter: EnumType = {
      id: 'e1',
      schema: 'public',
      name: 'order_status',
    ...
    expect(sql).toContain('ALTER TYPE "public"."order_status" ADD VALUE IF NOT EXISTS \'shipped\';')
  })
  ...
  it('detects dropped table-level CHECK constraint', () => {
    ...
    const sql = flattenDiffSql(renderDiffSql(diff, [], [after]))
    expect(sql).toContain('ALTER TABLE "public"."events" DROP CONSTRAINT IF EXISTS "chk_dates";')
  })
  ...
  it('same-name index with changed columns → drop + add', () => {
    const before = table({
      ...
      indexes: [{id: 'i1', name: 'idx_models', columns: [{fieldId: 'fa'}], isUnique: false}],
    })
    const after = table({
      id: 't1',
      name: 'models',
    ...
    const createIdx = sql.findIndex(s =>
      s.startsWith('CREATE INDEX "idx_models" ON "public"."models" ("a", "b")')
    )
    expect(dropIdx).toBeGreaterThanOrEqual(0)
  ...
  it('same-name index with isUnique toggled → drop + add', () => {
    const before = table({
      ...
      name: 't',
      fields: [field({id: 'fa', name: 'a', type: 'INTEGER'})],
      indexes: [{id: 'i1', name: 'idx_t_a', columns: [{fieldId: 'fa'}], isUnique: false}],
    })
    const after = table({
      id: 't1',
      name: 't',
      fields: [field({id: 'fa', name: 'a', type: 'INTEGER'})],
      indexes: [{id: 'i1', name: 'idx_t_a', columns: [{fieldId: 'fa'}], isUnique: true}],
    })
    const diff = computeSchemaDiff(project({tables: [before]}), project({tables: [after]}))
