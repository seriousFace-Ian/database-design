...
import {Button, Checkbox, Dropdown, Empty, Input, Space, Table, theme, Tooltip} from 'antd'
...
function buildPresetMenu(fieldType: string, onPick: (val: string) => void): MenuProps {
...
const FieldsTable: React.FC<Props> = ({table}) => {
  ...
  const columns: ColumnsType<FieldDefinition> = [
    ...
    },
    {
      ...
      fixed: 'left',
      render: (_, record) => (
        <FieldNameCell field={record} onChange={name => updateField(table.id, record.id, {name})} />
      ),
    },
    {
      title: '类型',
        ...
        <Space size={2}>
          ...
          <Tooltip title={record.foreignKey ? '编辑外键' : '添加外键'}>
            <Button
              icon={
                <LinkOutlined
                  style={{color: record.foreignKey ? token.colorPrimary : token.colorTextDisabled}}
                />
              }
              size="small"
              type="text"
  ...
  return (
    <>
      ...
      {fkField && (
        <ForeignKeyModal open field={fkField} tableId={table.id} onClose={() => setFkField(null)} />
      )}
